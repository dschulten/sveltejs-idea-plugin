...
//            else if(type == SvelteTokenTypes.N_ATTR) parseNAttr(builder);
    ...
    private void parseTemplate(PsiBuilder builder) {
        ...
        // is there a name?
        String tagName = null;
        if(builder.getTokenType() == SvelteTokenTypes.BLOCK_STMT_NAME) {
            Marker macroNameMark = builder.mark();
            tagName = builder.getTokenText();
            builder.advanceLexer();
            macroNameMark.done(SvelteTokenTypes.BLOCK_STMT_NAME);
        }
...
//        parseParams(tagName, builder, SvelteTokenTypes.CLOSING);
        ...
        paramsMark.done(SvelteTokenTypes.SVELTE_PARAMS);
